...
import { AGENT_LIST_PAGE_SIZE } from "@/lib/pagination";
...
function ManageBookings() {
  ...
  const [bookedHasMore, setBookedHasMore] = useState(false);
  const [cancelledHasMore, setCancelledHasMore] = useState(false);
  const [pendingHasMore, setPendingHasMore] = useState(false);
  const [loadingMore, setLoadingMore] = useState(false);
  const [streamTotal, setStreamTotal] = useState(0);

  async function fetchOwnBookingPages(
    pages: { booked: number; cancelled: number; pending: number },
    append: boolean,
  ) {
    const [booked, cancelled, pending] = await Promise.all([
      api.getBookedTickets(pages.booked, AGENT_LIST_PAGE_SIZE),
      api.getCancelledTickets(pages.cancelled, AGENT_LIST_PAGE_SIZE),
      api.getPendingBankPayments(pages.pending, AGENT_LIST_PAGE_SIZE),
    ]);
    const chunk = [
      ...booked.data.map(fromTicket),
      ...cancelled.data.map(fromTicket),
      ...pending.data.map(fromPendingBooking),
    ];
    setOwnRows((prev) => (append ? [...prev, ...chunk] : chunk));
    setBookedHasMore(booked.hasMore);
    setCancelledHasMore(cancelled.hasMore);
    setPendingHasMore(pending.hasMore);
    setBookedPage(pages.booked);
    setCancelledPage(pages.cancelled);
    setPendingPage(pages.pending);
    setStreamTotal(booked.total + cancelled.total + pending.total);
  }

  useEffect(() => {
    setLoading(true);
    fetchOwnBookingPages({ booked: 1, cancelled: 1, pending: 1 }, false)
      .catch((err) => toast.error(err instanceof Error ? err.message : t("error_occured")))
      .finally(() => setLoading(false));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const listHasMore = bookedHasMore || cancelledHasMore || pendingHasMore;

  async function loadMoreOwnRows() {
    if (!listHasMore || loadingMore) return;
    setLoadingMore(true);
    try {
      await fetchOwnBookingPages(
        {
          booked: bookedHasMore ? bookedPage + 1 : bookedPage,
          cancelled: cancelledHasMore ? cancelledPage + 1 : cancelledPage,
          pending: pendingHasMore ? pendingPage + 1 : pendingPage,
        },
        true,
      );
    } catch (err) {
    ...
    <div className="mx-auto max-w-6xl">
      ...
      {!isBusy && filtered.length ? (
        <>
          ...
          {!searchAllAgents && listHasMore ? (
            <ListLoadMore
              hasMore={listHasMore}
              loading={loadingMore}
              onLoadMore={loadMoreOwnRows}
              shown={ownRows.length}
              total={streamTotal}
            />
          ) : null}
        </>
